    ...
    def process_frame(self, frame):
        ...
        try:
                ...
                if mp_results.multi_face_landmarks:
                    ...
                    if len(f_check) > 0:
                        ...
                        if self.detect_head_silhouette(frame):
                            ...
                            if missing_duration >= 10: # Hair-trigger (Instant enforcement)
                                ...
                                # Only terminate after 5 major violations
                                if self.no_face_strike_count >= 5:
                                    self.record_event("TERMINATION_NO_PERSON", "Security Violation: Candidate not visible after multiple warnings. Interview terminated.", "CRITICAL", frame)
                                    result["current_warning"] = "TERMINATION: No person detected (5 strikes reached)!"
                                else:
                                    self.record_event("NO_PERSON", f"Security Warning ({self.no_face_strike_count}/5): Candidate not visible for 10s.", "HIGH", frame)
                                    result["face_detected"] = False
                                    result["current_warning"] = f"🔴 STAY IN FRAME {self.no_face_strike_count}/5!"
                ...
                if len(faces) == 0:
                     if self.detect_head_silhouette(frame):
                        ...
                        if missing_duration >= 10: # Hair-trigger (Instant enforcement)
                            ...
                            if self.no_face_strike_count >= 5: 
                                self.should_terminate = True
                                self.record_event("TERMINATION_NO_PERSON", "Security Violation: No person detected (5 strikes). Session ended.", "CRITICAL", frame)
                                result["current_warning"] = "TERMINATION: No person detected!"
                            else:
                                self.record_event("NO_PERSON", f"Security Warning ({self.no_face_strike_count}/5): No person detected for 10s.", "HIGH", frame)
                                result["face_detected"] = False
                                result["current_warning"] = f"🔴 STAY IN FRAME {self.no_face_strike_count}/5!"
            ...
            if self.model:
                ...
                if person_count > 1:
                    result["face_detected"] = True
                    # INSTANT TERMINATION for multiple persons
                    self.should_terminate = True
                    self.record_event("TERMINATION_MULTI_PERSON", f"Security Violation: {person_count} people detected. Session ended immediately.", "CRITICAL", frame, boxes=detected_boxes)
                    self.termination_reason = "Security Violation: Multiple persons detected (Zero Tolerance)."
                    result["current_warning"] = "TERMINATION: Multiple persons detected!"
                
                if gadget_detected:
                    # INSTANT TERMINATION for gadgets
                    self.should_terminate = True
                    self.record_event("TERMINATION_GADGET", f"Security Violation: Prohibited device ({detected_label}) detected. Session ended immediately.", "CRITICAL", frame, boxes=detected_boxes)
                    self.termination_reason = f"Security Violation: Prohibited device ({detected_label}) detected (Zero Tolerance)."
                    result["current_warning"] = "TERMINATION: Prohibited device detected!"
            ...
            if result.get("face_detected") and hasattr(self, 'reference_frame') and self.reference_frame is not None:
                ...
                # Check identity every 3 frames (~1.5 seconds)
                if self._identity_check_count >= 3: 
                    ...
                    if not matched:
                        ...
                        if "QUALITY_SKIP" in feedback:
                            ...
                            self.consecutive_identity_mismatch += 1
                            if self.consecutive_identity_mismatch >= 4: # ~6 seconds of mismatch
                                self.identity_mismatch_counts += 1
                                self.consecutive_identity_mismatch = 0
                                
                                if self.identity_mismatch_counts >= 3:
                                    self.should_terminate = True 
                                    self.record_event("TERMINATION_IDENTITY", "Security Violation: Identity mismatch consistently detected (3 strikes). Session ended.", "HIGH", frame)
                                    self.termination_reason = "Security Violation: Identity profile mismatch (3 strikes reached)."
                                    result["current_warning"] = "TERMINATION: Identity mismatch!"
                                else:
                                    self.record_event("IDENTITY_WARN", f"Security Warning ({self.identity_mismatch_counts}/3): Identity mismatch detected.", "HIGH", frame)
                                    result["current_warning"] = f"🔴 WARNING {self.identity_mismatch_counts}/3: Identity mismatch! Please look directly at the camera."
                    else:
                        self.consecutive_identity_mismatch = 0
                        if distance < 0.45: 
