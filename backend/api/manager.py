...
class InterviewManager:
    ...
    def _get_default_flow(self, plan_id=0):
        """Extended comprehensive flow with 20+ steps across all levels."""
        return [
            "greeting",
            "warmup",
            "resume_overview",
    ...
    def update_flow_for_plan(self, plan_id, practice_section=None):
        ...
        if self.practice_mode:
            if practice_section == 'projects':
                ...
                self.interview_flow = ["greeting", "conclusion"]
            print(f"🎯 [FLOW] Practice Mode Activated: {practice_section}")
        # 2. HANDLE STANDARD PLAN FLOW
        elif self.plan_id == 1:
            # Starter: greeting → resume skills/projects from CV → technical → coding → close
            self.interview_flow = [
                "greeting",
                "resume_overview",
                "resume_skills",
            ...
            print(f"📉 [FLOW] Reduced Interview Flow for Starter Plan (Plan 1)")
        elif self.plan_id == 2:
            # ATS Pro: greeting → resume (overview, skills, projects from CV) → scenarios → technical → coding → close
            self.interview_flow = [
                "greeting", 
                "resume_overview",
                "resume_skills", 
            ...
            print(f"📊 [FLOW] Standard Flow for ATS Pro Plan (Plan 2)")
        elif self.plan_id == 3:
            # Proctor Elite: greeting → resume from CV → full suite → HR wrap-up
            self.interview_flow = [
                "greeting", 
                "resume_overview",
                "resume_skills", 
            ...
            print(f"🛡️ [FLOW] Full Flow for Proctor Elite Plan (Plan 3)")
        elif self.plan_id == 4:
            # Ultimate Bundle (Plan 4): The exhaustive "Elite Ultra" flow
            self.interview_flow = self._get_default_flow()
            print(f"👑 [FLOW] Full Elite Flow Activated (Plan {self.plan_id})")
        else:
            # Default/Free/Unknown: 5 questions starting with resume
            self.interview_flow = [
                "greeting",
                "resume_overview",
                "resume_skills",
            ...
            print(f"📉 [FLOW] Demo Flow (5 questions) Activated (Plan {self.plan_id})")
        with self.lock:
            self.history = []
            self.evaluations = []
            self.submitted_solutions = []
    ...
    def load_resume(self, file_path):
        ...
        try:
            ...
            if not self._is_content_a_resume(text):
    ...
    def _is_content_a_resume(self, text):
        """Heuristic and AI check to ensure the document is a resume."""
        if not text or len(text) < 100: return False
        
        # 1. Heuristic: Look for common resume sections
        keywords = ["education", "experience", "skills", "projects", "employment", "internship", "technologies", "certifications"]
        ...
        # 2. AI Fallback: Ask if this is a resume
        if not self.client: return match_count >= 1 # Lenient if offline
        
        try:
            prompt = f"Is the following text from a Resume/CV? Return 'YES' or 'NO' only. Text snippet: {text[:1500]}"
    ...
    def _extract_entities(self, text):
        ...
        # Pre-clean text to avoid JSON noise
        clean_text = text[:4500].replace('{', '[').replace('}', ']')
        prompt = (
        ...
        try:
            ...
            print(f" [RESUME] Extracted: {len(self.skills_mentioned)} skills, {len(self.projects_mentioned)} projects.")
        except Exception as e:
            print(f" [RESUME] Entity extraction failed: {e}")
            # Fallback to simple regex for skills if LLM fails
            self.skills_mentioned = re.findall(r'\b(Python|Java|React|Node|SQL|AWS|Docker|Kubernetes|TensorFlow|PyTorch|C\+\+|Javascript)\b', text, re.I)

    def verify_candidate_match(self, name, resume_text):
        """Checks if registration name matches resume content strictly."""
        if not name or not resume_text: return False, "Missing info"
        
        # 1. Clean and normalize
        name_clean = str(name).strip().lower()
        # Avoid matching generic placeholder names
        if name_clean in ["guest user", "unknown", "candidate", "user", "n/a", "none"]:
             return True, name # Skip strict check for guest accounts to avoid frustration

        norm_name = re.sub(r'\s+', '', name_clean)
        # We search specifically in the first 1000 characters where the name usually is
        header_text = resume_text[:1000].lower()
        norm_header = re.sub(r'\s+', '', header_text)

        # 2. Exact match check
        if norm_name in norm_header: 
            return True, name
        
        # 3. Component match check (requires at least two parts to match for robustness)
        parts = [p for p in name_clean.split() if len(p) > 2]
        matches = [p for p in parts if p in header_text]
        
        # If we have a multi-part name (First Last), require at least two parts or a significant part
        if len(parts) >= 2:
            if len(matches) >= 2:
                return True, name
        elif len(matches) >= 1:
            # Single name matching (risky but allowed)
            return True, name

        # 4. LLM Fallback (Strict prompt)
        if not self.client: return False, "Mismatch (Local)"
        
        prompt = (
            f"Candidate registered as: '{name}'.\n"
            f"Resume snippet (first 2000 chars):\n{resume_text[:2000]}\n\n"
            f"TASK: Does the name on the resume match '{name}'? "
            f"If it matches or is a clear variation (e.g. 'Rob' for 'Robert'), return 'YES: [Full Name]'. "
            f"If it is a different person OR no name is found, return 'NO'. "
            f"Be strict. If the resume belongs to someone else, say NO."
        )
        try:
            res = self.client.chat.completions.create(
                model=self.model_name, 
            ...
            print(f"Error in verify_candidate_match LLM call: {e}")
    ...
    def create_overall_pie_chart(self, filename="overall_pie_chart.png"):
        """Create a professional Donut Chart with a central score label."""
        ...
        category_map = {
            ...
            'Coding': ['Coding'],
            'Strategy': ['Intro', 'Warmup', 'Conclusion', 'Case Study']
        }
        grouped_scores = {}
        ...
        colors_list = ['#1d4ed8', '#059669', '#7c3aed', '#db2777', '#ea580c', '#0891b2']
        
        overall_pct = (sum(sizes) / total_possible) * 100
        ...
        wedges, texts, autotexts = ax.pie(
            ...
            colors=colors_list,
            pctdistance=0.82,
            wedgeprops=dict(width=0.28, edgecolor='w', linewidth=3)
        )
        
        # Center Label
        ax.text(0, 0, f'{overall_pct:.0f}%\nTotal', ha='center', va='center', fontsize=28, weight='bold', color='#0f172a')
        ...
        ax.legend(wedges, labels, title="Performance Domains", loc="center", bbox_to_anchor=(0.5, -0.05), ncol=2, frameon=False)
        ax.set_title("ASSESSMENT DOMAIN DISTRIBUTION", pad=20, fontsize=14, weight='black', color='#1e293b')
        fig.savefig(filename, dpi=300, bbox_inches='tight')
        return filename
    ...
    def generate_pdf_report(self, filename, plan_id=0):
        ...
        avg_grammar = sum(self.sf(e.get('grammar_score', e.get('fluency', 0))) for e in evals_copy) / t_q if evals_copy else 0
        ...
        sc_val = ParagraphStyle('ScVal', fontSize=18, textColor=C_MAIN, fontName='Helvetica-Bold', alignment=TA_CENTER)
        sc_sub = ParagraphStyle('ScSub', fontSize=8, textColor=C_MUTED, fontName='Helvetica-Bold', alignment=TA_CENTER, tracking=1)
        card = [
            [
                Table([[safe_para(f"{ov_score:.1f}", sc_val), safe_para("OVERALL GRADE", sc_sub)]], colWidths=[3.2 * inch]),
                Table([[safe_para(f"{avg_corr:.1f}", sc_val), safe_para("TECH ACCURACY", sc_sub)]], colWidths=[3.2 * inch]),
            ],
            [
                Table([[safe_para(f"{avg_fluency:.1f}", sc_val), safe_para("COMMUNICATION", sc_sub)]], colWidths=[3.2 * inch]),
                Table([[safe_para(f"{proctor_pct:.0f}%", sc_val), safe_para("INTEGRITY SCORE", sc_sub)]], colWidths=[3.2 * inch]),
            ],
        ]
        ...
        story.append(KeepTogether([safe_para("KEY PERFORMANCE INDICATORS", s_head), sc_tab]))
        ...
        # --- FINAL RECOMMENDATION SECTION (Screenshot 3) ---
    ...
    def generate_question(self, category='general', previous_answer=None):
        ...
        if category == 'greeting':
            context = "Step 1: Initial Greeting (Refreshing)"
            nm = candidate_name_copy or self.candidate_name or "the candidate"
            context_instruction = (
                f"Greet {nm} professionally and warmly. Ask exactly ONE short welcoming question—specifically asking how they are doing today or how they are feeling."
            )
            self.current_topic = "Greeting"
        elif category == 'warmup':
            context = "Step 2: Human Rapport (Refreshing)"
            context_instruction = (
                "Continue the friendly opening. Acknowledge their reply, then ask exactly ONE question about how their day has been so far or if they had a good start to their morning/afternoon."
            )
    ...
    def evaluate_answer(self, question, answer):
        ...
        8. In "grammar_score", provide a score from 0-10 based on technical and linguistic accuracy.
        9. In "grammar_issues", list only MAJOR technical misstatements. Ignore all phonetic and transcription errors.
        Question: {question}
        ...
        try:
            ...
            if 'grammar_score' not in result: result['grammar_score'] = result.get('fluency', 5)
