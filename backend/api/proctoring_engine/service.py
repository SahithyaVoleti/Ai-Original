...
class ProctoringService:
    ...
    def start(self):
        ...
        self.multi_person_strike_count = 0
        self.gadget_strike_count = 0
    ...
    def record_event(self, event_type, message, severity="MEDIUM", frame=None, boxes=None):
        ...
        if severity == "CRITICAL_TERMINATE_NOW": # Reserved for future manual override only
    ...
    def verify_eyes(self, frame):
        ...
        try:
            ...
            # Relaxed EAR threshold for maximum compatibility
            if avg_ear < 0.10: 
                return False, f"Eyes not clearly detected or closed. Please look at the camera."
    ...
    def compare_profiles(self, profile_frame, live_frame):
        """
        Comprehensive Identity Verification (Biometric > Geometry > Histogram).
        Uses face_recognition for 99% accuracy if available.
        """
        try:
            # 1. Analyze Quality
            ...
            # 2. PRIORITY 1: Biometric Embedding (face_recognition)
            if has_face_rec:
                matched, dist, fb = self._compare_face_rec(profile_frame, live_frame)
                if matched:
                    return True, dist, "Identity Verified (Biometric)", is_low_light
                if fb and "STAGE_FAILED" not in fb:
                    # Clear mismatch from high-fidelity engine
                    return False, dist, fb, is_low_light
                # If stage failed (face missing in one frame), fall back to geometry/hist

            # 3. PRIORITY 2: Facial Geometry (MediaPipe)
            if has_mediapipe:
                geo_ok, geo_dist, geo_fb = self._compare_landmarks(profile_frame, live_frame)
                if geo_ok:
                    return True, geo_dist, "Identity Verified (Geometry)", is_low_light

            # 4. PRIORITY 3: Lightweight Histogram (OpenCV)
            ...
            # Threshold: 0.65 for histogram fallback (more forgiving than geometry)
            if similarity > 0.65:
        ...
        try:
            ...
            if has_mediapipe and self.face_mesh:
                ...
                if mp_results.multi_face_landmarks:
                    ...
                    # Multi-face check disabled to prevent false positives from background patterns
                    # MediaPipe is now used strictly for single-face landmarks and eye tracking
                    ...
                    if denom != 0:
                        ...
                        if h_ratio < 0.10 or h_ratio > 0.90:
                            self.consecutive_looking_away += 1
                            if self.consecutive_looking_away >= 10: 
                    ...
                    # Centering Check (Relaxed to 0.10-0.90)
                    if nose.x < 0.10 or nose.x > 0.90:
                         # result["current_warning"] = "⚠️ Please center yourself" # SILENCED
                         pass
                         
                    # Added: Eye Tracking / Gaze Check (Strict requirement)
                    eyes_ok, eye_msg = self.verify_eyes(frame)
                    if not eyes_ok:
                         # result["current_warning"] = f"⚠️ {eye_msg}" # SILENCED
                         pass
                    ...
                    if len(f_check) > 0:
                        ...
                        if self.detect_head_silhouette(frame):
                            ...
                            if missing_duration >= 30: # Increased from 20s for maximum tolerance
                                ...
                                if self.no_face_strike_count >= 3:
                                    ...
                                    self.record_event("NO_PERSON", f"Security Warning ({self.no_face_strike_count}/3): Candidate not visible for 30s.", "HIGH", frame)
                                ...
                                result["face_detected"] = False
                                result["current_warning"] = f"⚠️ WARNING: Stay in frame! ({int(30 - missing_duration)}s remaining)"
                ...
                if len(faces) == 0:
                     if self.detect_head_silhouette(frame):
                        ...
                        if missing_duration >= 30: # Use 30s for maximum tolerance
                            if not hasattr(self, 'no_face_strike_count'): self.no_face_strike_count = 0
                            self.no_face_strike_count += 1
                            
                            if self.no_face_strike_count >= 5: # Increased to 5 strikes
                                self.record_event("NO_PERSON_SESSION_ISSUE", "Security Issue: No person detected for a prolonged period. Continuing session but recording for review.", "HIGH", frame)
                                result["current_warning"] = "🔴 WARNING: No person detected for over 2 minutes!"
                            else:
                                self.record_event("NO_PERSON", f"Security Warning ({self.no_face_strike_count}/3): No person detected for 30s.", "HIGH", frame)
                                result["face_detected"] = False
                                result["current_warning"] = f"🔴 WARNING {self.no_face_strike_count}/3: Please stay in frame!"
                            ...
                            result["face_detected"] = False
                            result["current_warning"] = f"⚠️ WARNING: Stay in frame! ({int(30 - missing_duration)}s)"
                        elif missing_duration >= 2:
                            result["face_detected"] = False
                            result["current_warning"] = "⚠️ Face not detected — stay centered in front of the camera."
                        else:
                    ...
                    if len(faces) > 1:
                        ...
                        # WARNING ONLY in fallback mode: Haarcascades are prone to false positives
                        if self.consecutive_multi_face >= 15: # ~7.5 seconds
                            self.consecutive_multi_face = 0
                            self.record_event("MULTI_FACE_WARN", "Security Warning: Multiple face-like patterns detected (Fallback). Please check your background.", "MEDIUM", frame)
                            result["current_warning"] = "⚠️ Multiple faces detected (Fallback)? Please check background."
                    else:
                        if self.consecutive_multi_face > 0: self.consecutive_multi_face -= 1
            ...
            if self.model:
                ...
                for r in results:
                    for box in r.boxes:
                        try:
                            ...
                            if cls_id == 0 and conf > 0.80: # Ultra-High threshold to ensure NO false person detections
                                person_count += 1
                ...
                if person_count > 1:
                    ...
                    if self.consecutive_yolo_people >= 20: # ~10 seconds
                        self.multi_person_strike_count += 1
                        self.consecutive_yolo_people = 0
                        
                        if self.multi_person_strike_count >= 10: 
                            self.record_event("MULTI_PERSON_Sustained", f"Security Alert: {person_count} people detected consistently. Recorded for review.", "HIGH", frame, boxes=detected_boxes)
                            # Silenced on-screen alert to avoid distracting the candidate
                        else:
                            self.record_event("MULTI_PERSON_WARN", f"Security Warning ({self.multi_person_strike_count}/5): {person_count} people detected.", "HIGH", frame, boxes=detected_boxes)
                            # Silenced on-screen alert to avoid distracting the candidate
                else:
                ...
                if gadget_detected:
                    ...
                    if self.consecutive_phone >= 15: # ~7.5 seconds
                        if not hasattr(self, 'gadget_strike_count'): self.gadget_strike_count = 0
                        self.gadget_strike_count += 1
                        self.consecutive_phone = 0
                        
                        if self.gadget_strike_count >= 3:
                            self.should_terminate = True
                            self.should_terminate = True
                            self.record_event("TERMINATION_GADGET", f"Security Violation: Prohibited device ({detected_label}) detected consistently. Session ended.", "HIGH", frame, boxes=detected_boxes)
                            self.termination_reason = f"Security Violation: Prohibited device ({detected_label}) detected (3 strikes reached)."
                            result["current_warning"] = f"TERMINATION: Prohibited device detected!"
                        else:
                            self.record_event("GADGET_WARN", f"Security Warning ({self.gadget_strike_count}/3): Prohibited device ({detected_label}) detected.", "HIGH", frame, boxes=detected_boxes)
                            result["current_warning"] = f"🔴 WARNING {self.gadget_strike_count}/3: {detected_label} detected!"
                else:
                        ...
                        if "QUALITY_SKIP" in feedback:
                            ...
                            if self.consecutive_identity_mismatch >= 5: # Relaxed from 4
                                ...
                                if self.identity_mismatch_counts >= 3:
                                    self.should_terminate = True # 3 Strikes
                                    self.record_event("IDENTITY_ISSUE_Sustained", f"Identity Verification Alert: Continuous mismatch. Recorded for review.", "HIGH", frame)
                                    result["current_warning"] = f"🔴 ALERT: Identity mismatch sustained! Please adjust lighting/position."
