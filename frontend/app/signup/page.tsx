...
import { Send, User, ChevronRight, ShieldCheck, CheckCircle, Sun, Moon, Camera, RefreshCw, ArrowRight, Sparkles, FileText, Eye, EyeOff, Mic, MicOff } from 'lucide-react';
...
export default function Signup() {
    ...
    const chatStepRef = useRef(0); // Using a ref to prevent race conditions during fast clicks
        ...
        if (typeof window !== 'undefined') {
            ...
            if (SpeechRecognition) {
                ...
                recognition.onresult = (event: any) => {
                    ...
                    // Update the input field in real-time as the user speaks
    ...
    const toggleListening = () => {
        ...
        if (isListening) {
            ...
            // Stop Atlas from speaking if he's talking
            if (isSpeaking) {
                try { window.speechSynthesis.cancel(); } catch(e) {}
    ...
    useEffect(() => {
        ...
        if (lastMessage && lastMessage.sender === 'bot') {
            // Speak string content OR the text property if it's a JSX element wrapper
            const speechText = typeof lastMessage.text === 'string' 
                ? lastMessage.text 
                : (lastMessage as any).speechText || "";
    ...
    const handleOptionSelect = (val: string) => {
        ...
        // Always use the ref to get the absolute latest step index
        processStep(chatStepRef.current, val);
    };

    const processStep = async (currentStep: number, value: string) => {
        let responseText: string | React.ReactNode = '';
        let speechText = ''; // For TTS support on JSX messages
        ...
        switch (currentStep) {
            case 1: // Name Input -> Next is Email
                if (value.length < 2) {
                    setIsTyping(false);
            ...
            case 2: // Email Input -> Next is Phone
            ...
            case 3: // Phone Input -> Next is Domain
                const phoneClean = value.replace(/\D/g, '');
                if (phoneClean.length !== 10 || !/^[6-9]/.test(phoneClean)) {
                    setIsTyping(false);
                    setIsProcessing(false);
                    setMessages(prev => [...prev, { id: Date.now() + Math.random(), sender: 'bot', text: "Please enter a valid 10-digit mobile number starting with 6, 7, 8, or 9.", time: getCurrentTime() }]);
                    return;
                }
                currentData.phone = phoneClean;
                setUserData(prev => ({ ...prev, phone: phoneClean }));
                speechText = "Got it! Now, what is your educational domain?";
                responseText = (
            ...
            case 4: // Domain Input -> Next is Branch
                currentData.domain = value;
                setUserData(prev => ({ ...prev, domain: value }));
                responseText = `Excellent! 🎓 Within ${value}, what is your specialization or branch? (e.g. CSE, ECE, Pharma, Agronomy)`;
                break;

            case 5: // Branch Input -> Next is Year
            ...
            case 6: // Year Input -> Next is College
            ...
            case 7: // College Input -> Next is Password
                if (value.length < 3) {
                    setIsTyping(false);
                    setIsProcessing(false);
            ...
            case 8: // Password Input -> Next is Photo
                if (value.length < 6) {
                    setIsTyping(false);
                    setIsProcessing(false);
                    setMessages(prev => [...prev, { id: Date.now() + Math.random(), sender: 'bot', text: "Please use a stronger password (at least 6 characters).", time: getCurrentTime() }]);
                    return;
                }
        ...
        setTimeout(() => {
            setMessages(prev => [...prev, { 
                ...
                text: responseText, 
                time: getCurrentTime(),
                speechText: speechText || (typeof responseText === 'string' ? responseText : "")
            } as any]);
            setIsTyping(false);
            setIsProcessing(false);
            if (currentStep !== 9) {
    ...
    const confirmPhoto = () => {
        ...
        setIsTyping(true);
        setChatStep(10); // Moving to processing/final step
        ...
        const handleKeys = (e: KeyboardEvent) => {
            if (e.key === 'Enter') {
                if (!hasStarted) {
                    e.preventDefault();
                    startChat();
                } else if (chatStep === 9 && capturedImage) {
                    e.preventDefault();
                    confirmPhoto();
                } else if (chatStep === 11) {
                    e.preventDefault();
                                    ...
                                    <div className="relative flex items-center gap-2">
                                        <div className="relative flex-1 group">
                                            <input
                                                ...
                                                placeholder={isListening ? "Listening... Speak clearly" : getPlaceholder()}
                                                disabled={isTyping || chatStep > 8}
                                                autoComplete={chatStep === 8 ? "new-password" : "off"}
                                                className={`w-full bg-[var(--card-bg)] border border-[var(--border)] text-[var(--foreground)] placeholder-[var(--text-muted)] text-lg px-6 py-4 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:bg-[var(--background)] transition-all shadow-inner relative z-10 ${chatStep === 8 ? 'pr-14' : ''} ${isListening ? 'border-blue-500 ring-2 ring-blue-200' : ''}`}
                                                autoFocus
                                            />
                                            {chatStep === 8 && (
                                                <button
