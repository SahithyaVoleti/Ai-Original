...
export default function Dashboard() {
    ...
    const [drills, setDrills] = useState<{ case_studies: any[], behavioral: any[], projects: any[] }>({ case_studies: [], behavioral: [], projects: [] });
    const [selectedDrill, setSelectedDrill] = useState<any>(null);
    const [drillSearch, setDrillSearch] = useState('');
    const [activeDrillTab, setActiveDrillTab] = useState<'case_study' | 'behavioral' | 'projects'>('case_study');
    ...
    const scrollToLibrary = (section: string) => {
        ...
        const mapping: Record<string, 'case_study' | 'behavioral' | 'projects'> = {
            'projects': 'projects',
    ...
    useEffect(() => {
        ...
        const calculateSkills = () => {
            ...
            const normalizeCategory = (cat: string) => {
                ...
                if (c.includes('project') || c.includes('internship') || c.includes('experience')) return 'Expertise';
            ...
            const exp = getVal('Expertise');

            // --- PERFORMANCE TREND CALCULATION ---
            if (interviews.length >= 2) {
                const sortedInterviews = [...interviews].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
                const recent = sortedInterviews.slice(0, 3);
                const previous = sortedInterviews.slice(3, 6);
                
                const recentAvg = recent.reduce((sum, inv) => sum + (inv.overall_score || 0), 0) / Math.max(recent.length, 1);
                const prevAvg = previous.length > 0 
                    ? previous.reduce((sum, inv) => sum + (inv.overall_score || 0), 0) / previous.length 
                    : recentAvg * 0.9; // Default 10% gain if no previous data
            ...
            const newAchievements = achievements.map(ach => {
                ...
                if (ach.id === 'pioneer') unlocked = interviews.length > 0;
                if (ach.id === 'consistent') {
                    const days = new Set(interviews.map(inv => new Date(inv.date).toDateString()));
                    unlocked = days.size >= 3;
                }
                if (ach.id === 'technical') unlocked = tech >= 9; // average 9/10 or 90%
                if (ach.id === 'communication') unlocked = comm >= 9;
                if (ach.id === 'ats') unlocked = (user?.resume_score || 0) >= 80;
                if (ach.id === 'elite') unlocked = interviews.length >= 5 && (tech + dsa + comm + exp) / 4 >= 8;
                return { ...ach, unlocked };
            });
        ...
        const calculateDailyTrend = () => {
            // Transform to session-based trend for granular "Improvement Graph"
            ...
            return sorted.map((inv, idx) => {
                ...
                let prod = 0, serv = 0, acc = 0;
                
                try {
                    const details = typeof inv.details === 'string' ? JSON.parse(inv.details) : inv.details;
                    const evals = details.evaluations || [];
                    
                    let itTech = 0, itSolving = 0, itComm = 0, itExpert = 0;
                    let cTech = 0, cSolving = 0, cComm = 0, cExpert = 0;

                    evals.forEach((e: any) => {
                        const cat = normalizeCategory(e.type || 'General');
                        const s = parseFloat(e.score) || 5;
                        if (cat === 'Technical') { itTech += s; cTech++; }
                        if (cat === 'Problem Solving') { itSolving += s; cSolving++; }
                        if (cat === 'Communication') { itComm += s; cComm++; }
                        if (cat === 'Expertise') { itExpert += s; cExpert++; }
                    });

                    const tech = cTech > 0 ? itTech / cTech : 5;
                    const dsa = cSolving > 0 ? itSolving / cSolving : 5;
                    const comm = cComm > 0 ? itComm / cComm : 5;
                    const expert = cExpert > 0 ? itExpert / cExpert : 5;

                    prod = Math.round((tech * 0.4 + dsa * 0.4 + expert * 0.2) * 10);
                    serv = Math.round((comm * 0.4 + tech * 0.3 + expert * 0.3) * 10);
                    acc = evals.length > 0 ? (evals.reduce((a: any, b: any) => a + parseFloat(b.accuracy || b.correctness_score || b.correctness || 0), 0) / evals.length) * 10 : 0;
                } catch(e) {}

                return {
                    date: `${d}${sorted.filter(x => new Date(x.date).toDateString() === new Date(inv.date).toDateString()).length > 1 ? ` (${idx + 1})` : ''}`,
                    score: inv.overall_score || 0,
    ...
    const [performanceTrend, setPerformanceTrend] = useState({ value: 0, direction: 'up' });
    const [achievements, setAchievements] = useState([
        { id: 'pioneer', title: 'The Pioneer', desc: 'Completed your first interview.', icon: Play, unlocked: false },
        { id: 'consistent', title: 'Consistent Learner', desc: '3-day practice streak.', icon: Flame, unlocked: false },
        { id: 'technical', title: 'Technical Scholar', desc: 'Score 90% in technical round.', icon: Brain, unlocked: false },
        { id: 'communication', title: 'Communication Pro', desc: 'Expert level fluency analysis.', icon: MessageSquare, unlocked: false },
        { id: 'ats', title: 'ATS Master', desc: 'Resume verified & optimized.', icon: FileSearch, unlocked: false },
        { id: 'elite', title: 'Global Elite', desc: 'High proficiency across all rounds.', icon: Star, unlocked: false }
    ]);
    ...
    const getIntensityClass = (level: number) => {
        switch (level) {
            case 1: return 'bg-slate-200/50';
            case 2: return 'bg-slate-400';
            case 3: return 'bg-slate-900';
            default: return 'bg-slate-50 border border-slate-100/30';
        }
    };
    ...
    const SkillGauge = ({ value, label, color, icon: Icon }: any) => (
        <div className="bg-white/80 backdrop-blur-xl border border-slate-100 rounded-2xl p-5 flex flex-col items-center justify-center text-center group hover:bg-white hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-500 shadow-soft">
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        ...
                        <Pie
                            ...
                            paddingAngle={0}
                        >
                            <Cell fill={`url(#gradient-${label})`} />
                            <Cell fill="#f1f5f9" />
                        </Pie>
                    </PieChart>
                </ResponsiveContainer>
                <div className="absolute inset-0 flex items-center justify-center">
                    <Icon size={16} className="text-slate-400 group-hover:text-slate-900 transition-colors duration-500" />
                </div>
            </div>
            <div className="text-2xl font-black tracking-tighter text-slate-900 tabular-nums">{value}%</div>
            <div className="text-[9px] font-black uppercase text-slate-400 tracking-[0.2em] mt-2 group-hover:text-slate-600 transition-colors">{label}</div>
        </div>
    );

    const FitnessCircle = ({ data, title, dataKey }: any) => {
        const lastVal = data.length > 0 ? data[data.length - 1][dataKey] : 0;
        const prevVal = data.length > 1 ? data[data.length - 2][dataKey] : lastVal;
        const diff = lastVal - prevVal;
        
        return (
            <div className="bg-white/90 backdrop-blur-md border border-slate-100 rounded-[2rem] p-8 flex flex-col items-center justify-center shadow-soft group hover:shadow-2xl transition-all duration-500 relative overflow-hidden h-[420px]">
                {/* Decorative Background Elements */}
                <div className="absolute -top-24 -right-24 w-64 h-64 bg-slate-50 rounded-full blur-3xl opacity-50 group-hover:bg-blue-50 transition-colors duration-700"></div>
                <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-slate-50 rounded-full blur-3xl opacity-50 group-hover:bg-slate-100 transition-colors duration-700"></div>

                <div className="text-center mb-8 relative z-10">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight">{title}</h3>
                    <p className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em] mt-1.5">Industry Competency Index</p>
                </div>
                ...
                <div className="relative w-56 h-56 mb-8">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <defs>
                                <linearGradient id={`fitnessGradient-${dataKey}`} x1="0" y1="0" x2="1" y2="1">
                                    <stop offset="0%" stopColor="#0f172a" />
                                    <stop offset="100%" stopColor="#334155" />
                                </linearGradient>
                            </defs>
                            <Pie
                                data={[{ value: lastVal }, { value: 100 - lastVal }]}
                                innerRadius={82}
                                outerRadius={96}
                                startAngle={90}
                                endAngle={-270}
                                ...
                                cornerRadius={12}
                            >
                                <Cell fill={`url(#fitnessGradient-${dataKey})`} />
                                <Cell fill="#f1f5f9" />
                            </Pie>
                        </PieChart>
                    </ResponsiveContainer>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <div className="text-6xl font-black tracking-tighter text-slate-900 group-hover:scale-110 transition-transform duration-700 tabular-nums">{lastVal}%</div>
                        <div className={`flex items-center gap-2 mt-3 px-4 py-1.5 rounded-full border shadow-sm transition-all duration-500 ${diff >= 0 ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-rose-50 border-rose-100 text-rose-600'}`}>
                            {diff >= 0 ? (
                                <TrendingUp size={16} />
                            ) : (
                                <Activity size={16} />
                ...
                <div className="mt-auto w-full pt-6 flex items-center justify-between border-t border-slate-50 z-10">
                    <div className="flex flex-col">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Global Ranking</span>
                        <span className="text-xs font-bold text-slate-900 mt-0.5">Top 12% in Region</span>
                    </div>
                    <span className="px-5 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-black transition-colors cursor-default shadow-lg shadow-slate-200">
                        Target 85%
                    </span>
                </div>
            </div>
    ...
    const IndustryReadinessView = () => {
        const avgCorrectness = skillsData.length > 0 && !skillsData[0].name.includes('No Data')
            ? (skillsData.reduce((a, b) => a + (b.accuracy || 0), 0) / skillsData.length) 
            : 0;
        ...
        return (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <SkillGauge label="Expertise" value={skillsData.find(d => d.name === 'Expertise')?.value || 0} color="#64748b" icon={Award} />
                </div>

                {/* CORRECTNESS HUD SECTION - PREMIUM GLASS CARD */}
                <div className="bg-white/90 backdrop-blur-md border border-slate-100 rounded-[2rem] p-8 flex items-center justify-between shadow-soft group hover:shadow-2xl transition-all duration-700 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-2 h-full bg-slate-900 group-hover:w-3 transition-all duration-500"></div>
                    <div className="absolute top-[-50%] right-[-10%] w-64 h-64 bg-slate-50 rounded-full blur-3xl opacity-40"></div>
                    
                    <div className="flex items-center gap-6 relative z-10">
                        <div className="p-4 bg-slate-900 text-white rounded-2xl shadow-xl shadow-slate-200 group-hover:scale-110 transition-transform duration-500">
                            <CheckCircle2 size={24} />
                        </div>
                        <div>
                            <h3 className="text-xl font-black text-slate-900 tracking-tight">Average Accuracy</h3>
                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em] mt-2">Quality Engagement Index • AI Verified</p>
                        </div>
                    </div>
                    
                    <div className="flex flex-col items-end relative z-10">
                        <div className="text-5xl font-black text-slate-900 tracking-tighter tabular-nums">{avgCorrectness.toFixed(0)}%</div>
                        <div className="text-[9px] font-black uppercase text-emerald-500 tracking-widest mt-1">Above Peer Average</div>
                    </div>
                </div>

                <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <FitnessCircle title="Product-Based Fit" data={trendData} dataKey="productScore" />
                        <FitnessCircle title="Service-Based Fit" data={trendData} dataKey="serviceScore" />
                    </div>

                    <div className="p-6 bg-slate-50/80 backdrop-blur-sm border border-slate-100 rounded-2xl shadow-inner group hover:bg-white transition-colors duration-500">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-[9px] font-black uppercase tracking-[0.15em]">
                            <div className="flex items-center justify-between p-3 bg-white rounded-xl shadow-sm border border-slate-50">
                                <span className="text-slate-400">Product Fit Model:</span>
                                <code className="text-slate-900 font-mono">40%Tech + 40%Solve + 20%Exp</code>
                            </div>
                            <div className="flex items-center justify-between p-3 bg-white rounded-xl shadow-sm border border-slate-50">
                                <span className="text-slate-400">Service Fit Model:</span>
                                <code className="text-slate-900 font-mono">40%Comm + 30%Tech + 30%Exp</code>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="min-h-screen bg-[var(--background)] flex font-sans transition-all duration-500 selection:bg-slate-200 selection:text-slate-900">
            ...
            <aside className={`fixed lg:relative z-40 h-screen transition-all duration-700 ease-in-out ${isSidebarOpen ? 'w-80' : 'w-24'} ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                <div className="h-full m-5 mr-0 bg-white/90 backdrop-blur-xl border border-slate-100 rounded-[3rem] shadow-soft flex flex-col p-8 overflow-hidden group/sidebar">
                    <Link href="/" className={`mb-16 flex items-center transition-all duration-500 hover:opacity-80 ${isSidebarOpen ? 'gap-5 px-4' : 'justify-center'}`}>
                        <div className="w-14 h-14 bg-slate-900 rounded-[1.25rem] flex items-center justify-center text-white shadow-2xl shadow-slate-200 shrink-0 transform group-hover/sidebar:rotate-12 transition-transform duration-500">
                            <Zap size={28} fill="currentColor" />
                        </div>
                        {isSidebarOpen && (
                            <div className="flex flex-col animate-in fade-in slide-in-from-left-4 duration-700">
                                <span className="text-2xl font-black tracking-tighter text-slate-900 leading-none uppercase">Vignan AI</span>
                                <span className="text-[9px] font-black uppercase text-slate-400 tracking-[0.3em] mt-2">Elite Assessment Hub</span>
                            </div>
                        )}
                    </Link>
                    ...
                    <nav className="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-2">
                        {[
                            ...
                            <button
                                ...
                                className={`w-full flex items-center transition-all duration-500 rounded-[1.25rem] group/nav relative ${activeTab === item.name
                                    ? 'bg-slate-900 text-white shadow-2xl shadow-slate-200 scale-[1.02]'
                                    : item.isBypass ? 'text-amber-600 hover:bg-amber-50 border border-amber-100 mt-4' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-900'
                                    } ${isSidebarOpen ? 'px-6 py-4 gap-4' : 'justify-center py-4'}`}
                            >
                                <item.icon size={22} className={`${activeTab === item.name ? 'text-white' : 'group-hover/nav:text-slate-900'} transition-all duration-500`} />
                                {isSidebarOpen && <span className="font-bold text-sm tracking-tight">{item.name}</span>}
                                {activeTab === item.name && (
                                    <div className="absolute right-0 w-1 h-6 bg-white/40 rounded-full -translate-x-3" />
                                )}
                            </button>
                        ))}
                    </nav>
                ...
                <main className="flex-1 p-6 md:p-8 space-y-6 overflow-y-auto w-full max-w-[1500px] mx-auto scroll-smooth">
                        ...
                        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
                            ...
                            <div ref={libraryRef} className="mt-20 space-y-10 pb-10">
                                ...
                                <div className="relative group">
                                    ...
                                    <input
                                        type="text"
                                        placeholder={`Search ${activeDrillTab === 'case_study' ? 'Case Studies' : activeDrillTab === 'behavioral' ? 'Behavioral' : 'Project Defense'}...`}
                                        value={drillSearch}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {(activeDrillTab === 'case_study' ? drills.case_studies : activeDrillTab === 'behavioral' ? drills.behavioral : drills.projects)
                                    ...
                                    ) : (activeDrillTab === 'case_study' ? drills.case_studies : activeDrillTab === 'behavioral' ? drills.behavioral : drills.projects).length === 0 ? (
                                        <div className="col-span-full py-20 text-center space-y-4">
                    ...
                    {activeTab === 'Dashboard' ? (
                        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
                                ...
                                <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-[2rem] border border-slate-200 bg-gradient-to-r from-slate-900 to-slate-800 text-white px-8 py-6 shadow-soft">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-white/75 mt-1">Unlimited interview sessions on this subscription</p>
                                        </div>
                                    </div>
                                    <button
                                        type="button"
                            ...
                            {/* PREMIUM WELCOME HEADER & HUD */}
                            <div className="flex flex-col gap-10">
                                <div className="flex flex-col lg:flex-row justify-between items-center gap-10 bg-white/90 backdrop-blur-xl border border-slate-100 rounded-[3rem] p-12 md:p-16 shadow-soft relative overflow-hidden group">
                                    <div className="absolute top-[-50%] right-[-10%] w-[500px] h-[500px] bg-slate-50 rounded-full blur-3xl opacity-50 group-hover:scale-110 transition-transform duration-1000"></div>
                                    
                                    <div className="relative flex flex-col md:flex-row items-center gap-12 text-center md:text-left z-10 flex-1">
                                        <div className="relative group/photo w-36 h-36 md:w-52 md:h-52 shrink-0 rounded-full bg-white border-[10px] border-white flex justify-center items-center shadow-2xl overflow-hidden cursor-pointer transition-all hover:scale-105 duration-700 ring-1 ring-slate-100">
                                            {user?.photo ? (
                                                <img src={user.photo} className="w-full h-full object-cover" alt="Profile" />
                                            ) : (
                                                <span className="text-6xl font-black text-slate-900">{userInitial}</span>
                                            )}
                                            <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover/photo:opacity-100 flex items-center justify-center transition-all backdrop-blur-md">
                                        ...
                                        <div className="space-y-6">
                                            <div className="inline-flex items-center gap-2 px-5 py-2 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-[0.25em] shadow-xl shadow-slate-200">
                                            ...
                                            <h1 className="text-5xl md:text-8xl font-black tracking-tighter leading-[0.85] text-slate-900">
                                                Welcome back,<br />
                                                <span className="text-slate-900/90">{user.name.split(' ')[0]}!</span>
                                            </h1>
                                            <p className="text-slate-400 text-lg md:text-2xl font-bold max-w-xl leading-relaxed mt-6">
                                                {interviews.length > 0 
                                                    ? `Your performance is trending ${performanceTrend.direction} ${performanceTrend.value}% this week. Ready to reach the next tier?`
                                                    : "Welcome! Your journey to a premium career starts here. Begin your first assessment to unlock analytics."}
                                            </p>
                                        </div>
                                    </div>
                                    
                                    <div className="flex flex-col gap-5 relative z-10 shrink-0 w-full lg:w-auto">
                                        <button onClick={() => { enterFullScreen(); router.push('/?start=true'); }} className="bg-slate-900 text-white hover:bg-black px-14 py-7 rounded-[2.5rem] font-black text-sm tracking-[0.2em] flex items-center justify-center gap-5 transition-all shadow-2xl shadow-slate-300 group/btn active:scale-95">
                                            START ASSESSMENT <ArrowRight size={22} className="group-hover:translate-x-1 transition-transform" />
                                        </button>
                                        <button onClick={() => setActiveTab('Analytics')} className="bg-white border border-slate-100 text-slate-900 hover:bg-slate-50 px-14 py-7 rounded-[2.5rem] font-black text-sm tracking-[0.2em] flex items-center justify-center gap-5 transition-all shadow-soft active:scale-95">
                                            VIEW ANALYTICS <BarChart3 size={22} />
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                                    {[
                                        { label: 'Overall Score', value: `${averageScore.toFixed(0)}%`, sub: 'Peer Benchmarked', icon: Target },
                                        { label: 'Sessions', value: stats.total, sub: 'Lifetime Attempts', icon: Activity },
                                        { label: 'Accuracy', value: `${skillsData.find(d => d.name === 'Technical')?.accuracy || 0}%`, sub: 'Engagement Quality', icon: Zap },
                                        { label: 'Consistency', value: 'Level 3', sub: 'Weekly Momentum', icon: Flame },
                                    ].map((s, i) => (
                                        <div key={i} className="bg-white/80 backdrop-blur-xl border border-slate-100 rounded-[2rem] p-8 shadow-soft flex items-center gap-6 group hover:bg-white hover:shadow-2xl transition-all duration-700">
                                            <div className="p-4 bg-slate-50 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all duration-700 rounded-2xl shadow-sm">
                                                <s.icon size={26} />
                                            </div>
                                            <div>
                                                <div className="text-3xl font-black text-slate-900 tracking-tighter tabular-nums">{s.value}</div>
                                                <div className="text-[10px] font-black uppercase text-slate-400 tracking-[0.15em] mt-1.5 group-hover:text-slate-600 transition-colors">{s.label}</div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="max-h-[600px] overflow-y-auto custom-scrollbar">
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-slate-50">
                                            {interviews.length === 0 ? (
                                                <tr><td colSpan={5} className="p-20 text-center text-slate-500 font-bold uppercase tracking-widest">No assessment records found.</td></tr>
                                            ) : (
                                                interviews.map((inv, i) => (
                                                    <tr key={i} className="hover:bg-slate-50/80 transition-all cursor-default group/row">
                                                        <td className="px-6 py-6 text-center text-[11px] font-black text-slate-300 group-hover/row:text-slate-900 transition-colors">{String(i + 1).padStart(2, '0')}</td>
                                                            ...
                                                            <div className="flex items-center gap-6">
                                                                <div className="relative w-12 h-12">
                                                                    <ResponsiveContainer width="100%" height="100%">
                                                                        <PieChart>
                                                                            <Pie
                                                                                data={[{ value: inv.overall_score || 0 }, { value: 100 - (inv.overall_score || 0) }]}
                                                                                innerRadius={16}
                                                                                outerRadius={22}
                                                                                startAngle={90}
                                                                                endAngle={-270}
                                                                                ...
                                                                                cornerRadius={4}
                                                                            >
                                                                                <Cell fill={inv.overall_score >= 80 ? '#0f172a' : '#475569'} />
                                                                                <Cell fill="#f1f5f9" />
                                                                            </Pie>
                                                                        </PieChart>
                                                                    </ResponsiveContainer>
                                                                ...
                                                                <div className="flex flex-col">
                                                                    <span className={`text-[10px] font-black uppercase tracking-widest ${inv.overall_score >= 80 ? 'text-emerald-500' : 'text-slate-400'}`}>
                                                                        {inv.overall_score >= 80 ? 'Exceptional' : 'Standard'}
                                                                    </span>
                                                                    <span className="text-[9px] font-bold text-slate-400 mt-0.5">Verified Protocol</span>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-10 py-6">
                                                            <div className="flex flex-col">
                                                                <span className="font-bold text-slate-900 text-sm group-hover/row:text-blue-600 transition-colors">{inv.module_name || 'General Assessment'}</span>
                                                                <div className="flex items-center gap-2 mt-1.5">
                                                                    <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border ${inv.status === 'completed' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                                            inv.status === 'terminated' ? 'bg-rose-50 text-rose-700 border-rose-100' :
                                                                                'bg-slate-50 text-slate-500 border-slate-100'
                                                                        }`}>
                                                                        {inv.status || 'started'}
                                                                    </span>
                                                                    {inv.integrity_score >= 90 && (
                                                                        <span className="bg-blue-50 text-blue-700 border border-blue-100 px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest">High Integrity</span>
                                                                    )}
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-10 py-6 text-[11px] text-slate-500 font-bold uppercase tracking-tight tabular-nums">{new Date(inv.date).toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                                                        <td className="px-10 py-6 text-right flex items-center justify-end gap-3">
                                                            <button
                                                                ...
                                                                className="p-3 bg-white border border-slate-100 text-slate-400 rounded-xl hover:bg-blue-50 hover:text-blue-600 hover:border-blue-100 transition-all shadow-sm group-hover/row:shadow-md"
                                                            ...
                                                            <button
                                                                ...
                                                                className="p-3 bg-white border border-slate-100 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all shadow-sm group-hover/row:shadow-md"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                                {achievements.map((badge, i) => (
                                     <div key={i} className={`group p-10 rounded-[2.5rem] border bg-white transition-all duration-500 ${badge.unlocked ? 'border-blue-200 shadow-soft hover:shadow-md hover:border-blue-300' : 'border-slate-200 shadow-sm hover:border-slate-300'}`}>
                                         <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-8 border transition-transform group-hover:scale-105 ${badge.unlocked ? 'border-blue-200 bg-blue-50 text-blue-950' : 'border-slate-100 bg-slate-50 text-slate-400'}`}>
                                             <badge.icon
                                                 size={28}
                                                 strokeWidth={1.25}
                                                 fill="currentColor"
                    ...
                    {activeTab === 'Analytics' ? (
                        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                            {/* ANALYTICS HEADER */}
                            <div className="p-12 bg-slate-900 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
                                <div className="absolute top-[-20%] right-[-10%] w-96 h-96 bg-blue-500/20 rounded-full blur-3xl"></div>
                                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                                    <div>
                                        <h1 className="text-4xl font-black tracking-tight mb-3">Deep Performance Analytics</h1>
                                        <p className="text-white/60 font-medium max-w-lg">Advanced algorithmic analysis of your interview competencies, consistency, and growth trajectories.</p>
                                    </div>
                                    <div className="flex gap-4">
                                        <div className="bg-white/10 backdrop-blur-md px-6 py-4 rounded-2xl border border-white/10 text-center min-w-[120px]">
                                            <div className="text-3xl font-black tabular-nums">{averageScore.toFixed(1)}%</div>
                                            <div className="text-[9px] font-black uppercase tracking-widest text-white/40 mt-1">Global Avg</div>
                                        </div>
                                        <div className="bg-white/10 backdrop-blur-md px-6 py-4 rounded-2xl border border-white/10 text-center min-w-[120px]">
                                            <div className="text-3xl font-black tabular-nums">{stats.total}</div>
                                            <div className="text-[9px] font-black uppercase tracking-widest text-white/40 mt-1">Sessions</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                <div className="bg-white/90 backdrop-blur-md border border-slate-100 rounded-[2.5rem] p-10 shadow-soft group hover:shadow-2xl transition-all duration-500">
                                    <div className="flex items-center justify-between mb-10">
                                        <h3 className="text-xl font-black flex items-center gap-4 text-slate-900">
                                            <div className="p-3 bg-slate-900 text-white rounded-2xl shadow-lg"><Activity size={20} /></div> Performance Spectrum
                                        </h3>
                                        <span className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Multi-Dimensional Analysis</span>
                                    </div>
                                    <div className="h-[400px]">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <RadarChart data={granularMetrics.length > 0 ? granularMetrics : skillsData}>
                                                <PolarGrid stroke="#f1f5f9" />
                                                <PolarAngleAxis dataKey="name" tick={{ fontSize: 11, fontWeight: 900, fill: '#64748b', letterSpacing: '0.05em' }} />
                                ...
                                <div className="bg-white/90 backdrop-blur-md border border-slate-100 rounded-[2.5rem] p-10 shadow-soft group hover:shadow-2xl transition-all duration-500">
                                    <div className="flex items-center justify-between mb-10">
                                        <h3 className="text-xl font-black flex items-center gap-4 text-slate-900">
                                            <div className="p-3 bg-slate-100 text-slate-900 rounded-2xl group-hover:bg-slate-900 group-hover:text-white transition-colors"><TrendingUp size={20} /></div> Growth Trajectory
                                        </h3>
                                        <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${performanceTrend.direction === 'up' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                            {performanceTrend.direction === 'up' ? '+' : '-'}{performanceTrend.value}% Trend
                                        </div>
                                    </div>
                                    <div className="h-[400px]">{PerformanceTrendLine()}</div>
                                </div>
                            </div>

                            <div className="bg-white/90 backdrop-blur-md border border-slate-100 rounded-[2.5rem] p-10 shadow-soft overflow-hidden group hover:shadow-2xl transition-all duration-500">
                                <div className="flex items-center justify-between mb-10">
                                    <div className="flex items-center gap-4">
                                        <div className="p-3 bg-slate-50 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-colors rounded-2xl"><Calendar size={20} /></div>
                                        <div>
                                            <h3 className="text-xl font-black text-slate-900 tracking-tight">Consistency Momentum</h3>
                                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest mt-1">Annual Activity Visualization</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-2 items-center">
                                        <span className="text-[9px] font-black uppercase text-slate-300 mr-2">Intensity:</span>
                                        <div className="w-3.5 h-3.5 rounded-[3px] bg-slate-50 border border-slate-100"></div>
                                        <div className="w-3.5 h-3.5 rounded-[3px] bg-slate-200/50"></div>
                                        <div className="w-3.5 h-3.5 rounded-[3px] bg-slate-400"></div>
                                        <div className="w-3.5 h-3.5 rounded-[3px] bg-slate-900"></div>
                                    </div>
                                </div>
