...
import { Sun, Moon, Shield, ShieldAlert, Camera, Mic, Monitor, User, CheckCircle, Check, AlertCircle, LogOut, Sparkles, FileText, BarChart, ArrowRight, ArrowLeft, Volume2, ChevronRight, Brain, LayoutDashboard, Play, Terminal, Video, Zap, Globe, Lock, Database, Loader, Send } from 'lucide-react';
import { useTheme } from './theme-context';
import Editor from "@monaco-editor/react";

const apiBase = typeof window !== 'undefined' ? (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000') : (process.env.INTERNAL_BACKEND_URL || 'http://backend-api:5000');
const proctorBase = typeof window !== 'undefined' ? (process.env.NEXT_PUBLIC_PROCTOR_URL || 'http://localhost:5001') : (process.env.INTERNAL_PROCTOR_URL || 'http://backend-worker:5001');
...
function HomeContent() {
  ...
  const buildPhases = (planId: number = 0, section: string | null = null, mode: string | null = null) => {
    ...
    if (mode === 'practice' && section) {
      if (section === 'intro' || section === 'self_intro') return ['greeting', 'warmup', 'conclusion'];
    ...
    if (pId === 2) { // ATS Pro
      return [
        'greeting', 'warmup', 'warmup', 'resume_skills', 'resume_projects', 'resume_overview',
    ...
    // Default/Free/Unknown: Optimized flow based on user request
    return [
      'greeting', 
      'warmup', 'warmup', // 2 General questions
      'resume_projects', // Projects
      'scenario_behavioral', 'scenario_technical', // Scenarios
      'scenario_hr', 'hr_culture', // HR
      'technical_core', 'technical_advanced', // Technical deep-dive
      'code', 'code',   // Coding round
      'conclusion'
    ];
  };
  ...
  useEffect(() => {
    ...
    if (saved) {
      setWorkflowSessionId(saved);
      // Auto-resume check: only if we're on landing and haven't already resumed
      if (stage === 'landing' && !hasAutoResumed) {
        fetch(`${apiBase}/api/session/state?session_id=${saved}`)
          .then(res => res.json())
          .then(data => {
            if (data.status === 'success' && data.current_state === 'INTERVIEW_IN_PROGRESS') {
              const ctx = data.context || {};
              console.log("♻️ [RESUME] Active session detected. Restoring state:", ctx);
              
              if (ctx.last_category === 'code' || ctx.last_category === 'coding') {
                startCodingRound();
              } else {
                setStage('interview');
                setQuestion(ctx.last_question || '');
                questionRef.current = ctx.last_question || '';
                setPhase(ctx.last_category || 'warmup');
                setPhaseCount(ctx.question_count || 0);
                if (ctx.interview_id) setInterviewId(ctx.interview_id);
                
                // STRICT: Sync phase index
                const newPhases = buildPhases(user?.plan_id, searchParams?.get('section'), searchParams?.get('mode'));
                const idx = newPhases.indexOf(ctx.last_category || 'warmup');
                if (idx !== -1) setPhaseIndex(idx);
                
                setTimeout(() => {
                  speak("Resuming the interview. " + (ctx.last_question || ""));
                }, 2000);
              }
  ...
  }, [user]); // Re-run when user loads to ensure plan-based phases are ready
  ...
  useEffect(() => {
    // Start session recording when interview starts
    if (['interview', 'code'].includes(stage)) {
    ...
    const updateLevel = () => {
      if (analyserRef.current) {
        ...
        if (stage === 'interview' && !isSpeakingRef.current) {
          if (currentLevel > 2) { // Lowered for soft speech sensitivity
    ...
    try {
      ...
      if (activeStream && activeStream.getAudioTracks().length > 0) {
        ...
        try {
          ...
          // Create a recording stream that uses the BOOSTED audio if available
          const boostedStream = (window as any).__boostedAudioStream;
          const recordingStream = new MediaStream([
            ...(boostedStream ? boostedStream.getAudioTracks() : activeStream.getAudioTracks()),
    ...
    const init = async () => {
      if (isCameraNeeded) {
        ...
        try {
          ...
          // Audio Visualizer & Boost
          try {
            const audioCtx = new (window.AudioContext || (window as any).webkitAudioContext)();
            const source = audioCtx.createMediaStreamSource(stream);
            const gainNode = audioCtx.createGain();
            gainNode.gain.value = 5.0; // CRITICAL: Boosted sensitivity for very low voices
            ...
            // Create a boosted destination for the MediaRecorder
            const boostedDest = audioCtx.createMediaStreamDestination();
            gainNode.connect(boostedDest);
            (window as any).__boostedAudioStream = boostedDest.stream;
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onresult = (event: any) => {
          ...
          console.log("🎤 Final Add:", cleanFinalAdd, "Interim:", currentInterim);
        ...
        recognition.onstart = () => {
          ...
          console.log("🎤 Mic is ACTIVE / Listening (Stage: " + stage + ")");
        };
    ...
    const watchdog = setInterval(() => {
      ...
      // SAFETY: Force isSpeakingRef to false if it's been stuck for too long (browser synthesis glitch)
      if (isSpeakingRef.current && (!lastSpeechTimeRef.current || (now - lastSpeechTimeRef.current > 20000))) {
         console.warn("⚠️ [VOICE GUARD] isSpeakingRef stuck - force resetting");
         isSpeakingRef.current = false;
         setIsSpeaking(false);
      }

      if (now % 5000 < 2000) { // Log every ~5s to avoid spam
        console.log("🔍 Mic Watchdog:", { stage, recognitionActive: recognitionActiveRef.current, shouldMicBeOff, isSpeaking: isSpeakingRef.current, isTranscribing, fetchingQuestion });
      }
      ...
      const whisperTranscript = await new Promise<string>((resolve) => {
        recorder.onstop = async () => {
          ...
          try {
            ...
            const transRes = await fetch(`${apiBase}/api/transcribe`, {
              method: "POST",
              body: formData
            });
  ...
  const fetchNextQuestion = async (cat: string) => {
    ...
    try {
      ...
      if (data.interview_id) {
        ...
        console.log(" 🆔 Received Interview ID from backend:", data.interview_id);
      }
  ...
  const handleSubmitCode = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/submit_code`, {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleTermination = (reason: string) => {
    ...
    speak(reason, async () => {
      ...
      try {
        ...
        if (data.status === "success") {
          ...
          setStage("results");
          fetch(`${proctorBase}/proctor/stop`, { method: "POST" }).catch(() => {});
        }
    ...
    // Stop and upload session recording
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      if (isExited) {
        ...
        fetch(`${proctorBase}/proctor/event`, {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const res = await fetch(`${proctorBase}/proctor/status?session_id=${workflowSessionId}`);
        const data = await res.json();
    ...
    const sendFrame = async () => {
      ...
      try {
        const res = await fetch(`${proctorBase}/proctor/process_frame`, {
  ...
  const speak = (text: string, onComplete?: () => void, bypassDeferral = false) => {
    ...
    */
    ...
    // HARD GUARD: Mute microphone hardware while Agent is speaking (Acoustic Echo Guard)
    const activeStream = streamRef.current || currentStream || (window as any).__cameraStream;
    if (activeStream) {
      activeStream.getAudioTracks().forEach(track => { track.enabled = false; });
    }

    // 3. RAPID-FIRE / REDUNDANT DEFERRALS
    if (!bypassDeferral && (showFullscreenWarnRef.current || showTabSwitchWarnRef.current)) {
      ...
      // Actively request fullscreen to break the loop if needed
      if (!showFullscreenWarnRef.current && !showTabSwitchWarnRef.current) {
        document.documentElement.requestFullscreen().catch(() => { });
      }
    ...
    const playFallback = async () => {
      ...
      try {
        ...
        audio.onended = () => {
          ...
          // RE-ENABLE Microphone hardware after Agent finishes
          const activeStream = streamRef.current || currentStream || (window as any).__cameraStream;
          if (activeStream) {
            activeStream.getAudioTracks().forEach(track => { track.enabled = true; });
          }
  ...
  const startSessionRecording = () => {
    if (isSessionRecordingRef.current) return;
    
    const stream = streamRef.current || (window as any).__cameraStream;
    if (!stream) {
      console.warn("⚠️ No stream available for session recording.");
      return;
    }
    ...
    try {
      ...
      recorder.onstop = async () => {
        console.log("🎬 Session recording stopped. Preparing upload...");
        const blob = new Blob(sessionChunksRef.current, { type: mimeType });
        uploadSessionVideo(blob);
      };

      recorder.start(5000); // Record in 5s chunks
      sessionRecorderRef.current = recorder;
      isSessionRecordingRef.current = true;
      setIsSessionRecording(true);
      console.log(`🎥 Session recording STARTED with mimeType: ${mimeType}`);
    } catch (err) {
  ...
  const uploadSessionVideo = async (blob: Blob) => {
    ...
    const formData = new FormData();
    formData.append('video', blob, 'interview_session.webm');
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/upload_video`, {
        method: 'POST',
        body: formData
      });
      const data = await res.json();
  ...
  const handleBeginInterview = async (e?: any, bypass: boolean = false) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        // Start proctoring
        await fetch(`${proctorBase}/proctor/start`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        // Skip Identity Verification and go directly to Calibration
        speak(data.message || "Resume verified. Let's calibrate your environment.", () => {
      ...
      setFeedback("❌ Connection error. Please ensure backend services (API & Worker) are running.");
      console.error("Fetch Error Details:", e);
      if (e instanceof TypeError && e.message === "Failed to fetch") {
        console.warn("⚠️ It looks like the Proctoring Worker (Port 5001) might be offline. Please run 'python run_proctor_server.py' in the backend folder.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const captureAndVerify = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        fetch(`${proctorBase}/proctor/reset`, { method: "POST" }).catch(() => { });
        ...
        stage === 'landing' && (
          <div className="relative overflow-hidden">
            ...
            <section className="py-24 bg-slate-50 text-slate-900 border-t border-slate-200 relative overflow-hidden">
              {/* Noise texture removed to fix 404 */}
                ...
                <div className="flex-1 relative flex flex-col">
                  ...
                  <div className="absolute top-4 right-4 w-48 aspect-video bg-black/40 rounded-xl border border-white/20 shadow-2xl overflow-hidden z-30 pointer-events-none flex items-center justify-center relative">
                    <div className="absolute bottom-3 right-3 z-40 flex flex-col items-end gap-1.5">
                      {isSessionRecording && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-red-600/90 backdrop-blur-md rounded-lg border border-white/20 animate-pulse">
                          <div className="w-1.5 h-1.5 bg-white rounded-full"></div>
                          <span className="text-[7px] font-black uppercase tracking-[0.2em] text-white">REC</span>
                        </div>
                      )}
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <button
                  onClick={() => {
                    ...
                    hasStartedRef.current = true;
                    fetch(`${proctorBase}/proctor/reset`, { method: "POST" }).catch(() => { });
                    fetch(`${apiBase}/api/interview/reset`, { method: "POST" }).catch(() => { });
              ...
              <div className={`flex-[2.5] flex flex-col relative rounded-[3rem] overflow-hidden bg-white shadow-2xl border-4 transition-all duration-500 ${isSpeaking ? 'border-slate-400 shadow-slate-200' : 'border-slate-100'}`}>
                ...
                <div className="absolute top-4 left-4 right-4 md:top-10 md:left-10 md:right-10 flex justify-between items-start z-30 pointer-events-none">
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    {isSessionRecording && (
                       <div className="flex items-center gap-3 bg-red-600/20 backdrop-blur-md px-4 py-2 rounded-2xl border border-red-500/30 w-fit animate-pulse">
                         <div className="w-2 h-2 rounded-full bg-red-500"></div>
                         <span className="text-[10px] font-black text-red-500 uppercase tracking-widest">Assessment Recording Live</span>
                       </div>
                    )}
                    <button
                ...
                <div className="absolute bottom-12 left-0 right-0 z-40 flex flex-col items-center">
                  ...
                  <div className="w-full max-w-2xl px-6">
                      <div className="bg-slate-900/90 backdrop-blur-xl border border-white/20 pt-8 pb-10 px-10 rounded-[2.5rem] shadow-2xl pointer-events-auto transition-all animate-in slide-in-from-bottom-5 duration-500 flex flex-col items-center text-center max-w-3xl mx-auto ring-4 ring-white/10">
                        <div className="flex items-center gap-2 mb-4">
                          <div className="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></div>
                          <span className="text-[10px] font-black text-blue-300 uppercase tracking-[0.2em]">Interviewer Question</span>
                        </div>
                ...
                <div className="bg-white flex flex-col gap-4 flex-1 min-h-[350px] rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden group">
                  ...
                  <div ref={transcriptScrollRef} className="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar text-center flex flex-col items-center">
                    {!transcript && !interimTranscript ? (
                      ...
                      <div className="w-full h-auto min-h-[120px] py-6 px-4 bg-slate-50/30 rounded-[2rem] border border-slate-100/50 text-xl font-bold text-slate-800 leading-relaxed text-center whitespace-pre-wrap relative overflow-hidden">
                        <div className="absolute top-2 left-1/2 -translate-x-1/2 flex items-center gap-2 opacity-30">
                          <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-ping"></div>
                          <span className="text-[8px] font-black uppercase tracking-[0.2em]">Live Transcript</span>
                        </div>
                        {transcript}
                        {interimTranscript && (
                          <span className="text-indigo-600 opacity-80 italic">
                            {transcript ? " " : ""}{interimTranscript}
                          </span>
                        )}
                        {(!transcript && !interimTranscript) && (
                          <span className="text-slate-300 font-medium italic text-lg block mt-2">Start speaking when you are ready...</span>
                        )}
          ...
          <main className="min-h-screen flex items-center justify-center p-4 md:p-8 bg-[var(--background)]">
            <div className="w-full max-w-2xl bg-[var(--card-bg)] p-6 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] border border-[var(--border)] shadow-[0_30px_100px_rgba(0,0,0,0.2)] text-center relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-12">
                ...
                <div className="p-6 bg-[var(--background)] rounded-3xl border border-[var(--border)]">
                  <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-1">Resume ATS</div>
                  <div className="text-4xl font-black text-purple-600">{reportData?.resume_score || 0}%</div>
                </div>
                <div className="p-6 bg-[var(--background)] rounded-3xl border border-[var(--border)] hidden md:block">
                  <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-1">Status</div>
                  <div className={`text-xl font-black ${
                    (reportData?.score || 0) >= 80 ? 'text-green-600' :
                    (reportData?.score || 0) >= 60 ? 'text-blue-600' :
                    (reportData?.score || 0) >= 40 ? 'text-yellow-600' :
                    (reportData?.score || 0) > 0 ? 'text-orange-600' : 'text-red-600'
                  }`}>
                    {(reportData?.score || 0) >= 80 ? 'EXCELLENT' :
                     (reportData?.score || 0) >= 60 ? 'GOOD' :
                     (reportData?.score || 0) >= 40 ? 'AVERAGE' :
                     (reportData?.score || 0) > 0 ? 'NEEDS IMPROVEMENT' : 'NOT ATTEMPTED'}
                  </div>
                </div>
              </div>

              {reportData?.resume_analysis && (
                <div className="mb-12 text-left bg-indigo-50/30 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/30">
                  <h3 className="text-sm font-black text-indigo-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Sparkles size={16} /> ATS Analysis & Career Roadmap
                  </h3>
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-white/80 dark:bg-slate-900/50 p-4 rounded-2xl border border-white dark:border-white/5">
                      <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Detected Field</div>
                      <div className="text-xs font-bold text-slate-700 dark:text-slate-200">{reportData.resume_analysis.field}</div>
                    </div>
                    <div className="bg-white/80 dark:bg-slate-900/50 p-4 rounded-2xl border border-white dark:border-white/5">
                      <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Exp Level</div>
                      <div className="text-xs font-bold text-slate-700 dark:text-slate-200">{reportData.resume_analysis.level}</div>
                    </div>
                  </div>
                  
                  <div className="space-y-4">
                    <div>
                      <div className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-2">Skill Recommendations</div>
                      <div className="flex flex-wrap gap-2">
                        {reportData.resume_analysis.recommended_skills?.slice(0, 8).map((skill: string, i: number) => (
                          <span key={i} className="px-3 py-1 bg-white dark:bg-slate-800 rounded-lg text-[9px] font-bold text-slate-600 dark:text-slate-300 border border-slate-100 dark:border-slate-700">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      <div className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-2">Recommended Courses</div>
                      <div className="space-y-2">
                        {reportData.resume_analysis.courses?.map((course: any[], i: number) => (
                          <a key={i} href={course[1]} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 bg-indigo-600/5 hover:bg-indigo-600/10 rounded-xl transition-all border border-indigo-600/10 group">
                            <span className="text-[10px] font-bold text-slate-700 dark:text-slate-200">{course[0]}</span>
                            <Zap size={12} className="text-indigo-500 group-hover:scale-125 transition-transform" />
                          </a>
                        ))}
                      </div>
                    </div>
